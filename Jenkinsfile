		...
		stage('Create Schema in Atp') {
            steps {
                dir ('./sql') {
					...
					env.CHECK_SCHEMA = sh (script: 'exit | /opt/sqlcl/bin/sql -oci admin/${TF_VAR_database_password}@atpdb_HIGH \@./create_schema.sql', returnStdout: true).trim()
					sh 'echo ${CHECK_SCHEMA}'
                }
            }
        }
