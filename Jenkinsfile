pipeline {
	...
	parameters {
        string(defaultValue: "", description: 'What is the vault token ?', name: 'VAULT_TOKEN')
    ...
    stages {
        stage('Stage Init Atp Variables') {
            steps {
				...
				script {
					env.TENANCY_OCID = sh returnStdout: true, script: '''curl --header "X-Vault-Token: ${env.VAULT_TOKEN}" --request GET http://130.61.125.123:8200/v1/secret/demoatp | jq .data.tenancy_ocid | sed "s/\"//g"'''
				}
